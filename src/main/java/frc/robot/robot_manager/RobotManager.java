...
public class RobotManager extends StateMachine<RobotState> {
  ...
  protected RobotState getNextState(RobotState currentState) {
    ...
    return switch (currentState) {
      ...
      // Wait for deploy to move out of the way
      case PREPARE_ALGAE_INTAKE_FLOOR ->
          groundManager.deploy.atGoal() ? RobotState.ALGAE_INTAKE_FLOOR : currentState;
      ...
      case CORAL_INTAKE_LOLLIPOP_PUSH ->
          arm.atGoal() && elevator.atGoal() ? RobotState.STARTING_POSITION_CORAL : currentState;
  ...
  protected void collectInputs() {
    ...
    shouldLoopAroundToScoreObstruction =
        switch (getState()) {
          case ALGAE_NET_LEFT_WAITING, ALGAE_NET_LEFT_RELEASE -> ObstructionKind.LEFT_OBSTRUCTED;
          case ALGAE_NET_RIGHT_WAITING, ALGAE_NET_RIGHT_RELEASE -> ObstructionKind.RIGHT_OBSTRUCTED;
          default -> autoAlign.getObstruction();
        };
  ...
  public void intakeFloorAlgaeRequest() {
    if (!getState().climbingOrRehoming) {
      setStateFromRequest(RobotState.PREPARE_ALGAE_INTAKE_FLOOR);
    }
  }
